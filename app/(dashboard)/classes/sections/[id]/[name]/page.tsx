...
const SectionPage = ({ params }: { params: { id: string, name: string } }) => {
  return (
    ...
    <div>
      ...
      <AnimatedHeading className='font-misologist font-normal' title={`${decodeURIComponent(params.name).replace(/-/g, ' ')} Section`} varient='heading' />
  ...
  <SectionTable id={parseInt(params.id)}/>
  ...
  <AddSection classId={parseInt(params.id)}/>
  </Suspense>
  </div>
  )
}
