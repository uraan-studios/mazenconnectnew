...
const UpdateEmployee = ({employee, roles, statusOptions }: {employee: Employee; roles: Roles[]; statusOptions: StatusOption[] }) => {
    ...
    return (
      <Card className='max-w-xl mx-auto'>
        <CardHeader>
          <CardTitle>Editing Employee: {employee.name}</CardTitle>
          <CardDescription>
        ...
        <CardContent>
          <form onSubmit={onSubmit} className="space-y-2  justify-between w-full items-end bg-accent p-4 rounded-xl">
            ...
            <Button disabled= {loading} type="submit">{loading ? "Updating..." : "Update Employee"}</Button>
          </form>
          ...
          <div className="py-4">
            ...
            {success && (
              <Alert variant={'default'} className='bg-secondary'>
                <AlertCircle className="h-4 w-4" />
                <AlertTitle>Employee Updated! Redirecting.</AlertTitle>
